...
if detection:
    ...
    data = instance.BopInstanceDataset(**dataset_args,
                                       pbr=not args.real,
                                       test=args.real)
...
while True:
    ...
    with utils.timer('forward_mlp'):
        keys_verts = model.infer_mlp(
            torch.from_numpy(verts_norm).float().to(model.device),
            obj_idx)  # (N, emb_dim)
    ...
    for uv_name, uv_slice in zip(uv_names, uv_slices):
        view_uvs_unique, view_uvs_unique_inv = np.unique(uv_all[:, uv_slice],
                                                         axis=0,
                                                         return_inverse=True)
    ...
    for win_name in (*uv_names, 'dist', 'pose', 'mask_score', 'coord_score',
                     'keys'):
    ...
    def mouse_cb(event, x, y, flags=0, *_):
        ...
        for uv_name, uv_slice, (view_uvs_unique, view_uvs_unique_inv) in zip(
                uv_names, uv_slices, uv_uniques):
            uvs_unique_probs = np.zeros(len(view_uvs_unique),
                                        dtype=vert_probs.dtype)
            np.add.at(uvs_unique_probs, view_uvs_unique_inv, vert_probs)
            prob_img = np.zeros((224, 224, 3))
            yy, xx = view_uvs_unique.T
            prob_img[yy, xx, 2] = uvs_unique_probs / uvs_unique_probs.max(
            ) * p_mask.cpu().numpy()
            ...
            for p, c in zip(uv_pts_3d, np.eye(3)[::-1]):
                ...
                cv2.drawMarker(prob_img, tuple(p_uv[::-1]), c, cv2.MARKER_CROSS,
                               10)
    ...
    def estimate_pose():
        ...
        with utils.timer('pnp ransac'):
            (R, t, scores, mask_scores, coord_scores, dist_2d, size_mask,
             normals_mask) = pose_est.estimate_pose(
                 mask_lgts=mask_lgts,
                 query_img=query_img,
                 down_sample_scale=down_sample_scale,
                 obj_pts=verts,
        ...
        if not len(scores):
            ...
            R, t, scores, mask_scores, coord_scores = [
                a.cpu().numpy()
                for a in (R, t, scores, mask_scores, coord_scores)
            ]
