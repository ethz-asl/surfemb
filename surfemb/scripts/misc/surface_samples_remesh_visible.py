...
mesh_folder = Path('data/bop') / args.dataset / config[
    args.dataset].model_folder
...
for mesh_fp in tqdm(list(mesh_folder.glob('*.ply'))):
    ...
    ms.subdivision_surfaces_midpoint(iterations=10,
                                     threshold=pymeshlab.Percentage(
                                         args.remesh_percentage))
    ...
    area_reduction = trimesh.load_mesh(remesh_fp).area / trimesh.load_mesh(
        mesh_fp).area
    print()
    print(mesh_fp)
