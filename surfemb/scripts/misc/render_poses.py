...
pose_scores_fp = poses_fp.parent / (
    f'{name.replace("-depth", "")}-poses-scores.npy')
...
bboxes, det_obj_ids, scene_ids, det_scores, view_ids = [
    np.load(str(detection_folder / f'{name}.npy'))
    for name in ('bboxes', 'obj_ids', 'scene_ids', 'scores', 'view_ids')
]

start_idx = list(inst_count.keys()).index(
    args.scene_id) if args.scene_id is not None else 0
for scene_id, scene in list(inst_count.items())[start_idx:]:
    ...
    start_idx = list(scene.keys()).index(
        args.view_id
    ) if args.view_id is not None and scene_id == args.scene_id else 0
    for view_id, view in list(scene.items())[start_idx:]:
        ...
        img = cv2.imread(
            str(scene_folder / cfg.img_folder / f'{view_id:06d}.{cfg.img_ext}'))
        ...
        for obj_id, count in view.items():
            ...
            for bbox, det_score, pose, pose_score in zip(
                    bboxes[mask], det_scores[mask], poses[mask],
                    pose_scores[mask]):
                c = (0, 0, 255) if det_score < score_threshold else (0, 255, 0)
                if show_bbox:
                    l, t, r, b = bbox.round().astype(int)
                    cv2.rectangle(img, (l, t), (r, b), c)
                    cv2.putText(img, f'{obj_id}/{det_score:.3f}',
                                (l + 2, b - 2), cv2.FONT_HERSHEY_PLAIN, 1, c)
        ...
        render_vis = np.tile(
            cv2.cvtColor(img_, cv2.COLOR_BGR2GRAY)[..., None], (1, 1, 3)) / 255
        # print(render_vis.shape)
        render_vis[mask] = render[mask] * args.alpha + render_vis[mask] * (
            1 - args.alpha)
        if show_bbox:
            for obj_id, count in view.items():
                mask = (scene_ids == scene_id) & (view_ids == view_id) & (
                ...
                for bbox, pose_score in zip(bboxes[mask], pose_scores[mask]):
                    c = (0, 0, 1.) if pose_score < score_threshold else (0, 1.,
                                                                         0)
                    l, t, r, b = bbox.round().astype(int)
                    cv2.rectangle(render_vis, (l, t), (r, b), c)
                    cv2.putText(render_vis, f'{obj_id}/{pose_score:.3f}',
                                (l + 2, b - 2), cv2.FONT_HERSHEY_PLAIN, 1, c)
