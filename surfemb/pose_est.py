...
def estimate_pose(mask_lgts: torch.tensor,
    ...
    and scores pose hypotheses based on estimated mask and correspondence
    distribution.
    ...
    :param max_pose_evaluations: maximum number of poses to evaluate / score
        after pruning
    :param dist_2d_min: minimum 2d distance between at least one pair of
        correspondences for a hypothesis
    :param max_pool: max pool probs spatially to make score more robust (but
        less accurate), similar to a reprojection error threshold in common PnP
        RANSAC frameworks
    ...
    with timer('corr matrix', debug):
        if avg_queries:
            queries = F.avg_pool2d(query_img.permute(2, 0, 1),
                                   down_sample_scale).view(e, n).T  # (n, e)
            corr_matrix_log = torch.log_softmax(queries @ obj_keys.T,
                                                dim=1)  # (n, m)
            corr_matrix = corr_matrix_log.exp()
        else:
            # evaluate the whole corr_matrix followed by max pool for evaluation
            # (corr_matrix_log), batched to avoid oom use kernel centers for
            # corr_matrix
            query_img = query_img[:res_sampled *
                                  down_sample_scale, :res_sampled *
                                  down_sample_scale].permute(2, 0, 1)
            corr_matrix = torch.empty(res_sampled,
                                      res_sampled,
                                      m,
                                      device=device,
                                      dtype=torch.float32)
            ...
            n_patches_len = int(
                np.ceil(res_sampled * down_sample_scale / patch_len))
            ...
            for i in range(n_patches_len):
                ...
                for j in range(n_patches_len):
                    ...
                    shape = patch.shape[1:]
                    patch_corr_log = torch.log_softmax(
                        patch.reshape(e, -1).T @ obj_keys.T,
                        dim=1).view(*shape, m)
                    corr_matrix[to:bo, lo:ro] = patch_corr_log[
                        offset::down_sample_scale, offset::down_sample_scale]
                    corr_matrix_log[to:bo, lo:ro] = F.max_pool2d(
                        patch_corr_log.permute(2, 0, 1),
                        down_sample_scale).permute(1, 2, 0)
        ...
        if max_pool:
            ...
            corr_matrix_log = corr_matrix_log.view(res_sampled, res_sampled,
                                                   m).permute(2, 0,
                                                              1)  # (m, rs, rs)
            m_bs = 10000
            for i in range(int(np.ceil(m / m_bs))):
                l, r = i * m_bs, (i + 1) * m_bs
                corr_matrix_log[l:r] = F.max_pool2d(corr_matrix_log[l:r],
                                                    kernel_size=3,
                                                    stride=1,
                                                    padding=1)
    ...
    if poses is None:
        with timer('sample corr', debug):
            ...
            corr_matrix_cumsum = torch.cumsum(corr_matrix,
                                              dim=0,
                                              out=corr_matrix)
            corr_matrix_cumsum /= corr_matrix_cumsum[-1].item()
            # cumsum is overwritten. Reset variable to avoid accidental use
            corr_matrix = None
            corr_idx = torch.searchsorted(
                corr_matrix_cumsum, torch.rand(max_poses, 4,
                                               device=device))  # (max_poses, 4)
            del corr_matrix_cumsum  # frees gpu memory

            p2d_idx, p3d_idx = corr_idx.div(m,
                                            rounding_mode='floor'), corr_idx % m
            p2d, p3d = img_pts[p2d_idx].float(), obj_pts[
                p3d_idx]  # (max_poses, 4, 2 xy), (max_poses, 4, 3 xyz)
            n3d = obj_normals[
                p3d_idx[:, :3].cpu().numpy()]  # (max_poses, 3, 3 nx ny nz)
        ...
        with timer('pnp', debug):
            ...
            for i in range(max_poses):
                ret, rvecs, tvecs = cv2.solveP3P(p3d[i],
                                                 p2d[i],
                                                 K,
                                                 None,
                                                 flags=pnp_method)
        ...
        with timer('pose pruning', debug):
            # Prune hypotheses where all correspondences come from the same
            # small area in the image
            dist_2d = np.linalg.norm(p2d[:, :3, None] - p2d[:, None, :3],
                                     axis=-1).max(axis=(1, 2))  # (max_poses,)
            dist_2d_mask = dist_2d >= dist_2d_min * res_sampled

            # Prune hypotheses that are very close to or very far from the
            # camera compared to the crop
            z = poses[:, 2, 3]
            ...
            if do_prune:
                poses = poses[dist_2d_mask & size_mask &
                              normals_mask]  # (n_poses, 3, 4)
    else:
    ...
    def batch_score(R: torch.tensor, t: torch.tensor, visualize=False):
        n_poses = len(R)
        # project to image
        obj_pts_cam = obj_pts @ R.permute(0, 2, 1) + t[:,
                                                       None]  # (n_poses, m, 3)
        z = obj_pts_cam[..., 2]  # (n_poses, m)
        obj_pts_img = obj_pts_cam @ K.T
        u = (obj_pts_img[..., :2] /
             obj_pts_img[..., 2:]).round_()  # (n_poses, m, 2 xy)
        # ignore pts outside the image
        mask_neg = torch.any(torch.logical_or(u < 0, res_sampled <= u),
                             dim=-1)  # (n_poses, m)
        ...
        # maybe u should be rounded before casting to long - or converted to
        # long after rounding above but a small test shows that there are no
        # rounding errors
        u = u.long()

        # per pixel, find the vertex closest to the camera
        z, z_arg = torch_scatter.scatter_min(z, u, dim_size=n +
                                             1)  # 2x(n_poses, n + 1 ignore bin)
        # then discard the ignore bin: 2x(n_poses, n)
        z, z_arg = z[:, :-1], z_arg[:, :-1]
        # get mask of populated pixels
        mask = z > 0  # (n_poses, n)
        mask_pose_idx, mask_n_idx = torch.where(mask)  # 2x (k,)
        z, z_arg = z[mask_pose_idx, mask_n_idx], z_arg[mask_pose_idx,
                                                       mask_n_idx]  # 2x (k,)
        u = u[mask_pose_idx, z_arg]  # (k,)

        mask_score_2d = neg_mask_log_prob[None].expand(
            n_poses, n).clone()  # (n_poses, n)
        ...
        coord_score = torch_scatter.scatter_mean(coord_score,
                                                 mask_pose_idx,
                                                 dim_size=n_poses)  # (n_poses,)
        ...
        if visualize:
            assert len(R) == 1
            mask_score_img = mask_score_2d[0].view(
                res_sampled, res_sampled).cpu().numpy()  # [mi, 0]
            mask_score_img = 1 - mask_score_img / mask_score_img.min()
            cv2.imshow('mask_score', mask_score_img)

            coord_score_img = torch.zeros(res_sampled * res_sampled,
                                          3,
                                          device=device)
            coord_score_img[:, 2] = 1.
            coord_scores = corr_matrix_log[u, z_arg]  # (k_best)
            coord_score_img[u] = (1 - coord_scores / coord_scores.min())[:,
                                                                         None]
            cv2.imshow(
                'coord_score',
                coord_score_img.view(res_sampled, res_sampled, 3).cpu().numpy())
    ...
    if visualize and len(pose_scores) > 0:
        ...
        batch_score(R[best_pose_idx:best_pose_idx + 1],
                    t[best_pose_idx:best_pose_idx + 1],
                    visualize=True)
    ...
    return (R, t, pose_scores, mask_scores, coord_scores, dist_2d, size_mask,
            normals_mask)
