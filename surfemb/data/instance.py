...
class BopInstanceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if scene_ids is None:
            scene_ids = sorted(
                [int(p.name) for p in self.data_folder.glob('*')])
        for scene_id in tqdm(
            ...
            scene_gt_info = json.load(
                (scene_folder / 'scene_gt_info.json').open())
            scene_camera = json.load(
                (scene_folder / 'scene_camera.json').open())

            for img_id, poses in scene_gt.items():
                img_info = scene_gt_info[img_id]
                K = np.array(scene_camera[img_id]['cam_K']).reshape(
                    (3, 3)).copy()
                if pbr:
                    warnings.warn(
...
def _main():
    ...
    for pbr, test in (True, False), (False, False), (False, True):
        ...
        data = BopInstanceDataset(dataset_root=Path('bop/tless'),
                                  pbr=pbr,
                                  test=test,
                                  cfg=tless,
                                  obj_ids=range(1, 31))
